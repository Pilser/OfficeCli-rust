...
impl DocumentHandler for PptxHandler {
    ...
    fn add(&self, parent: &str, element_type: &str, position: InsertPosition, properties: &HashMap<String, String>) -> Result<String, HandlerError> {
        ...
        crate::add::add_element(&mut self.package.borrow_mut(), parent, element_type, position, properties)
    }
    ...
    fn move_element(&self, source: &str, target_parent: Option<&str>, position: InsertPosition) -> Result<String, HandlerError> {
        if !self.editable {
            return Err(HandlerError::OperationFailed("package opened in read-only mode".to_string()));
        }
    ...
    fn copy_from(&self, source: &str, target_parent: &str, position: InsertPosition) -> Result<String, HandlerError> {
        if !self.editable {
            return Err(HandlerError::OperationFailed("package opened in read-only mode".to_string()));
        }
        let mut pkg = self.package.borrow_mut();
    ...
    fn raw(&self, part_path: &str, _opts: RawOptions) -> Result<String, HandlerError> {
        self.package.borrow().read_part_xml(part_path)
    ...
    fn raw_set(&self, part_path: &str, xpath: &str, action: &str, xml: Option<&str>) -> Result<(), HandlerError> {
        ...
        crate::raw::apply_raw_set(&mut self.package.borrow_mut(), part_path, xpath, action, xml)
    }
    ...
    fn add_part(&self, parent: &str, part_type: &str, properties: Option<&HashMap<String, String>>) -> Result<(String, String), HandlerError> {
        ...
        crate::raw::add_part(&mut self.package.borrow_mut(), parent, part_type, properties)
    }
