...
pub fn extract_text_with_offsets(package: &oxml::OxmlPackage) -> Result<TextOffsetMap, HandlerError> {
    ...
    for slide in &pres.slides {
        ...
        map.push_span(&slide_header, &format!("/slide[{}]", slide.index), "slide-header");
        ...
        for (si, shape) in slide.shapes.iter().enumerate() {
            ...
            for (pi, para) in shape.paragraphs.iter().enumerate() {
                ...
                let para_path = format!("/slide[{}]/shape[{}]/paragraph[{}]", slide.index, si + 1, pi + 1);
