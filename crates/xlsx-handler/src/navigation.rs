...
pub fn parse_path(path: &str) -> Result<PathComponents, HandlerError> {
    ...
    let (main_path, property) = if path.contains(':') {
        ...
        if !["value", "formula", "style", "type", "ref"].contains(&prop) {
            return Err(HandlerError::InvalidPath(format!(
                "unknown property '{}': path '{}'", prop, path
            )));
        }
    ...
    let cell_ref_end = main_path.chars()
        .rev()
    ...
    let col_letters_len = main_path.chars()
        .rev()
    ...
    if total_len == cell_ref_len {
        ...
        let cr = CellRef::parse(ref_str)
            .ok_or_else(|| HandlerError::InvalidPath(format!("invalid cell reference '{}'", ref_str)))?;
...
pub fn get_node_at_path(
    ...
    match (pc.sheet_name, pc.cell_ref, pc.property) {
        ...
        (None, None, None) => {
            ...
            let children: Vec<DocumentNode> = model.sheets.iter().map(|ws| {
        ...
        (Some(sheet_name), None, None) => {
            ...
            let mut sheet_node = DocumentNode::new(&format!("/{}", ws.name), "sheet")
                .with_preview(format!("{} cells, cols 1-{}, rows 1-{}", ws.cells.len(), ws.max_col, ws.max_row));
        ...
        (Some(sheet_name), Some(cell_ref), None) => {
            ...
            let cell = ws.cells.get(&(cell_ref.row, cell_ref.col))
                .ok_or_else(|| HandlerError::PathNotFound(
                    format!("cell {}{}", sheet_name, cell_ref.to_string_ref())
                ))?;

            let mut cell_node = DocumentNode::new(
                &format!("/{}/{}", ws.name, cell.ref_str), "cell"
            )
                .with_text(cell.display_value.clone())
                .with_preview(cell.formula.clone().unwrap_or_default())
                .with_format("value", serde_json::Value::String(cell.raw_value.clone().unwrap_or_default()))
                .with_format("formula", serde_json::Value::String(cell.formula.clone().unwrap_or_default()))
                .with_format("type", serde_json::Value::String(cell_type_label(&cell.value_type).to_string()))
                .with_format("ref", serde_json::Value::String(cell.ref_str.clone()));

            if let Some(si) = cell.style_index {
                cell_node = cell_node.with_format("styleIndex", serde_json::Value::Number(si.into()));
            }
        ...
        (Some(sheet_name), Some(cell_ref), Some(prop)) => {
            ...
            Ok(DocumentNode::new(&path_str, "property")
                .with_text(value))
        }
        // Cell without sheet name is ambiguous
        (None, Some(cell_ref), prop) => {
        ...
        (Some(sheet_name), None, Some(prop)) => {
            Err(HandlerError::InvalidPath(format!(
