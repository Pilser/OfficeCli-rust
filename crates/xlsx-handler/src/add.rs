...
use crate::dom_types::*;
use crate::helpers;
use handler_common::{InsertPosition, HandlerError};
...
fn add_cell(
    ...
    let ref_str = properties.get("ref")
        .ok_or_else(|| HandlerError::InvalidArgument("cell requires 'ref' property (e.g. ref=B2)".to_string()))?;
    ...
    let cr = CellRef::parse(ref_str)
        .ok_or_else(|| HandlerError::InvalidArgument(format!("invalid cell reference '{}'", ref_str)))?;
    ...
    let cell_xml = if let Some(f) = &formula {
        ...
        if !t_attr.is_empty() { cell.push_str(&format!(" {}", t_attr)); }
        cell.push_str(&format!("><f>{}</f>", f));
        if !v_content.is_empty() { cell.push_str(&format!("<v>{}</v>", v_content)); }
        cell.push_str("</c>");
        cell
...
fn add_sheet(
    ...
    let name = properties.get("name")
        .ok_or_else(|| HandlerError::InvalidArgument("sheet requires 'name' property".to_string()))?;
    ...
    if model.sheets.iter().any(|s| s.name == *name) {
        return Err(HandlerError::InvalidArgument(format!("sheet '{}' already exists", name)));
    }
    ...
    package.write_part_xml(&part_path, &sheet_xml)
        .map_err(|e| HandlerError::SaveError(e.to_string()))?;
    ...
    let rels_xml = package.read_part_xml("xl/_rels/workbook.xml.rels")
        .map_err(|e| HandlerError::OperationFailed(format!("failed to read workbook rels: {}", e)))?;
