...
pub fn handle_info(
    ...
    match cmd.topic.as_deref() {
        ...
        Some("convert") => Ok("Convert legacy Office formats:\n  .doc -> .docx (Word)\n  .xls -> .xlsx (Excel)\n  .ppt -> .pptx (PowerPoint)\n  Also supports re-saving modern formats (.docx -> .docx, etc.)\n\n  Engines:\n    libreoffice (default) — high fidelity (~1:1), needs LibreOffice installed, slower\n    oxide — pure Rust, no install needed, fast but may lose styles/headers/objects\n\n  Usage: officecli convert old.doc\n  Options: -o/--output, --force, --engine libreoffice|oxide".to_string()),
