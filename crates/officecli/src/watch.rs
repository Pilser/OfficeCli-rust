...
#[derive(Debug)]
enum HandlerOp {
    ViewText { opts: ViewOptions },
    ViewAnnotated { opts: ViewOptions },
    ViewOutline,
    ViewStats,
    ViewHtml,
    Get { path: String, depth: usize },
    Query { selector: String },
    Set { path: String, properties: HashMap<String, String> },
    Add { parent: String, element_type: String, position: InsertPosition, properties: HashMap<String, String> },
    Remove { path: String },
    GetMark { path: String },
}
...
pub async fn run_server(file_path: &str, port: Option<u16>) -> Result<(), anyhow::Error> {
    ...
    let handler_thread = std::thread::spawn(move || {
        run_handler_thread(handler, op_rx);
    });
    ...
    tracing::info!("Watch server listening on http://127.0.0.1:{}/", actual_port);
...
fn run_handler_thread(
    handler: Box<dyn handler_common::DocumentHandler>,
...
fn execute_handler_op(handler: &dyn handler_common::DocumentHandler, op: HandlerOp) -> HandlerResult {
    match op {
        HandlerOp::ViewText { opts } => {
            match handler.view_as_text(opts) {
                Ok(t) => HandlerResult { data: serde_json::Value::String(t) },
                Err(e) => HandlerResult { data: serde_json::json!({"error": e.to_string()}) },
            }
        }
        HandlerOp::ViewAnnotated { opts } => {
            match handler.view_as_annotated(opts) {
                Ok(t) => HandlerResult { data: serde_json::Value::String(t) },
                Err(e) => HandlerResult { data: serde_json::json!({"error": e.to_string()}) },
            }
        }
        HandlerOp::ViewOutline => {
        ...
        HandlerOp::Get { path, depth } => {
            match handler.get(&path, depth) {
                Ok(node) => HandlerResult { data: serde_json::to_value(node).unwrap_or_default() },
                Err(e) => HandlerResult { data: serde_json::json!({"error": e.to_string()}) },
            }
        }
        HandlerOp::Query { selector } => {
            match handler.query(&selector) {
                Ok(nodes) => HandlerResult { data: serde_json::to_value(nodes).unwrap_or_default() },
                Err(e) => HandlerResult { data: serde_json::json!({"error": e.to_string()}) },
            }
        }
        HandlerOp::Set { path, properties } => {
        ...
        HandlerOp::Add { parent, element_type, position, properties } => {
            match handler.add(&parent, &element_type, position, &properties) {
                Ok(new_path) => HandlerResult { data: serde_json::json!({"path": new_path}) },
                Err(e) => HandlerResult { data: serde_json::json!({"error": e.to_string()}) },
            }
        }
        HandlerOp::Remove { path } => {
            match handler.remove(&path) {
                Ok(result) => HandlerResult { data: serde_json::json!({"removed": serde_json::to_value(result).unwrap_or_default()}) },
                Err(e) => HandlerResult { data: serde_json::json!({"error": e.to_string()}) },
            }
        }
        HandlerOp::GetMark { path } => {
            match handler.get(&path, 1) {
                Ok(node) => HandlerResult { data: serde_json::to_value(node).unwrap_or_default() },
                Err(e) => HandlerResult { data: serde_json::json!({"error": e.to_string()}) },
            }
        }
...
async fn send_op(state: &Arc<AppState>, op: HandlerOp) -> HandlerResult {
    ...
    if tx.send((op, reply_tx)).await.is_err() {
        return HandlerResult { data: serde_json::json!({"error": "handler thread closed"}) };
    }
    match reply_rx.await {
        Ok(result) => result,
        Err(_) => HandlerResult { data: serde_json::json!({"error": "handler thread dropped response"}) },
    }
}
...
async fn handle_index(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let text = result.data.as_str().unwrap_or("Error loading document");
...
#status {{ color: #569cd6; font-size: 0.8em; }}
.marked {{ background: #264f78; border-left: 3px solid #569cd6; padding-left: 4px; }}
</style>
</head>
<body>
<h2>{file_name}</h2>
...
es.onmessage = function(ev) {{
    ...
    try {{
        ...
        if (obj.text) {{
            document.getElementById('content').textContent = obj.text;
            document.getElementById('status').textContent = 'Updated ' + new Date().toLocaleTimeString();
        }}
        if (obj.mark) {{
            // highlight marked line (simple approach)
        }}
    }} catch(e) {{
        document.getElementById('content').textContent = ev.data;
        document.getElementById('status').textContent = 'Updated ' + new Date().toLocaleTimeString();
    }}
}};
es.onerror = function() {{
...
async fn handle_sse(
    ...
    let initial = send_op(&state, HandlerOp::ViewText { opts: ViewOptions::default() }).await;
...
async fn handle_mark(
    ...
    let result = send_op(&state, HandlerOp::GetMark { path: req.path.clone() }).await;

    if result.data.get("error").is_some() {
        Json(ApiResponse::err(result.data["error"].as_str().unwrap_or("unknown error")))
    } else {
...
async fn handle_view(
    ...
    let mode = params.get("mode").cloned().unwrap_or_else(|| "text".to_string());
    let opts = ViewOptions {
        start_line: params.get("start_line").and_then(|v| v.parse::<usize>().ok()),
        end_line: params.get("end_line").and_then(|v| v.parse::<usize>().ok()),
        max_lines: params.get("max_lines").and_then(|v| v.parse::<usize>().ok()),
        cols: params.get("cols").map(|c| c.split(',').map(|s| s.to_string()).collect()),
    };
...
async fn handle_get(
    ...
    let depth = params.get("depth").and_then(|v| v.parse::<usize>().ok()).unwrap_or(1);
...
async fn handle_set(
    ...
    let result = send_op(&state, HandlerOp::Set {
        path: req.path,
